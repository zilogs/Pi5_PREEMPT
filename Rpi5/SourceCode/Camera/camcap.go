...
func main() {
	...
	// Warm-up: touch a chunk of allocations sized like the steady-state
	// workload *before* we disable the GC and go real-time. This forces
	// the runtime to grow its heap arenas now, via ordinary (non-RT)
	// mmap calls, instead of doing it later mid-loop where an arena
	// grow would show up as a multi-hundred-microsecond jitter spike.
	warm := make([][]byte, 64)
	for i := range warm {
